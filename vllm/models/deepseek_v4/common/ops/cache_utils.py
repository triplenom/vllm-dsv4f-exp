...
def build_flashinfer_mixed_sparse_indices(
    ...
    padded_topk]`` (the first ``swa_index_width`` columns are SWA slot ids,
    the rest are compressed/top-k slot ids) and ``sparse_topk_lens`` (active
    length per token). Decode tokens read precomputed SWA/compressed indices;
    prefill tokens derive their SWA window from the position and translate
    local compressed indices to global slots via the block tables.

    ``swa_index_width`` is the dense width of ``decode_swa_indices`` and may
    exceed ``window_size`` for DSpark non-causal drafts; the logical sliding
    window stays ``window_size``.
    """
